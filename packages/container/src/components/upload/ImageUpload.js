...
const ImageUpload = ({
  ...
  useEffect(() => {
    if (defaultFilelist.length > 0) {
      if (multiple === false) {
        if (defaultFilelist[0].src || defaultFilelist[0].imageUrl) {
          setFileList([{
            uid: defaultFilelist[0].newFilename,
            status: 'success',
            url: defaultFilelist[0].src?? defaultFilelist[0].imageUrl,
            name: defaultFilelist[0].originalFilename,
            newFilename: defaultFilelist[0].newFilename,
            thumbUrl: defaultFilelist[0].src?? defaultFilelist[0].imageUrl,
          }]);
        }
      } else {
        const arr = [];
